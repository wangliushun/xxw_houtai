...
<body>
        ...
        <script type="text/javascript">
            function deletepl(cid){
                if(confirm("确认删除吗?")){
                    $.post("<%=basePath%>/joker/deleteplbyId",{'id':cid},function(data){
                        if(data > 0){
                            alert("删除成功");
                            sxpl();
                        }
            ...
            function sxpl(){
                $("#pl-table").bootstrapTable("refresh",{'pageNumber':1});
            }

            //页面分页展示娱乐话题
            function pl(){
               var jsid=${jokersid}
                $("#pl-table").bootstrapTable({
                    url:"<%=request.getContextPath()%>/joker/queryPlByJokerId?jokerid="+jsid,
                    striped: true,//隔行变色
